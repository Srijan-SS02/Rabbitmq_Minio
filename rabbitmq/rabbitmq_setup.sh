...
# Declare exchange
rabbitmqadmin --host=rabbitmq --username="$RABBITMQ_DEFAULT_USER" --password="$RABBITMQ_DEFAULT_PASS" declare exchange name="$RABBITMQ_EXCHANGE_NAME" type=direct durable=false

# Create queue
rabbitmqadmin --host=rabbitmq --username="$RABBITMQ_DEFAULT_USER" --password="$RABBITMQ_DEFAULT_PASS" declare queue name="$RABBITMQ_QUEUE_NAME" durable=false
